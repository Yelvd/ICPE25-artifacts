...
mpirun -n ${ntasks} --mca pml ucx -x UCX_NET_DEVICES=all --hostfile ./hostfile_$SLURM_JOBID --map-by ppr:${ppn}:node:PE=$JMP --bind-to hwthread likwid-perfctr -f -g CLOCK -g DATA -g L2 -g L3 -g MEM -o likwid_prof_rank_%r.txt ${binexe} config.xml | tee -a bull_log.out  
