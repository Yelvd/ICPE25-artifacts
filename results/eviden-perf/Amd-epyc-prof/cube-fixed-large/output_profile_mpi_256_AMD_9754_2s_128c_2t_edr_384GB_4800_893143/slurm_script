...
perf stat --all-cpus -o perf_report.log --event ex_ret_brn,ex_ret_brn_misp,cpu-cycles,cpu-clock,duration_time,ex_ret_instr,all_data_cache_accesses,ex_ret_instr,l2_request_g1.all_no_prefetch mpirun -n ${ntasks} --mca pml ucx -x UCX_NET_DEVICES=all --hostfile ./hostfile_$SLURM_JOBID --map-by ppr:${ppn}:node:PE=$JMP --bind-to hwthread ${binexe} config.xml | tee -a bull_log.out
