#!/bin/bash
#SBATCH -p AMD_9754_2s_128c_2t_edr_384GB_4800
#SBATCH -N 1
#SBATCH -t 6:0:0
#SBATCH -J HEMOCELL
#SBATCH --exclusive
...
WORKDIR=${PWD}/cube-${hemoconfig}/output_time_mpi_${ntasks}_${SLURM_JOB_PARTITION}_${SLURM_JOBID}
...
cp cube-${hemoconfig}/config.xml ${WORKDIR}/
...
/usr/bin/time -f %e -o bull_time.out mpirun -n ${ntasks} --mca pml ucx -x UCX_NET_DEVICES=all --hostfile ./hostfile_$SLURM_JOBID --map-by ppr:${ppn}:node:PE=$JMP --bind-to hwthread ${binexe} config.xml | tee -a bull_log.out

#/home_nfs/bhamitono/software/ipmi-energy-record/ipmi-energy-record.sh -p 0.5 -o ./report_${SLURM_JOBID}.csv -- srun ${binexe}  config.xml

#/home_nfs/bhamitono/software/ipmi-energy-record/ipmi-energy-record.sh -p 0.5 -o ./report_${SLURM_JOBID}.csv -- mpirun -n ${ntasks} --mca pml ucx -x UCX_NET_DEVICES=all --hostfile ./hostfile_$SLURM_JOBID --map-by ppr:${ppn}:node:PE=$JMP --bind-to hwthread ${binexe} config.xml
