...
WORKDIR=${PWD}/cube-${hemoconfig}/output_energy_mpi_${ntasks}_${SLURM_JOB_PARTITION}_${SLURM_JOBID}
...
cp /scratch_lustre_lscoe/users/bhamitono/hemocell/benchmark-setup/Intel-sapphire-rapids/wrapper_ddr.sh ${WORKDIR}/
...
/home_nfs/bhamitono/software/ipmi-energy-record/ipmi-energy-record.sh -p 0.5 -o ./report_${SLURM_JOBID}.csv -- mpirun -n ${ntasks} --mca pml ucx -x UCX_NET_DEVICES=all --hostfile ./hostfile_$SLURM_JOBID --map-by ppr:${ppn}:node:PE=$JMP --bind-to hwthread ./wrapper_ddr.sh ${binexe} config.xml | tee -a bull_log.out
