...
/usr/bin/time -f %e -o bull_time.out mpirun -n ${ntasks} --mca pml ucx -x UCX_NET_DEVICES=all --hostfile ./hostfile_$SLURM_JOBID --map-by ppr:${ppn}:node:PE=$JMP --bind-to hwthread ./wrapper_hbm.sh ${binexe} config.xml | tee -a bull_log.out
