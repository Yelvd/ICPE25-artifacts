...
WORKDIR=${PWD}/cube-${hemoconfig}/output_profile_mpi_${ntasks}_${SLURM_JOB_PARTITION}_${SLURM_JOBID}

# environment : compiler, mpi, ..
source $envfile
#module use /software/modulefiles/debug-profile/likwid
#module load aarch64/5.3.0
...
perf stat --all-cpus -o perf_report.log --event br_pred,instructions,cpu-cycles,duration_time,mem_access_rd,mem_access_wr,l1d_cache_wb,l2d_cache_wb,l1d_cache_refill,l1d_cache,l2d_cache,l2d_cache_refill,l3d_cache,l3d_cache_refill mpirun -n ${ntasks} --mca pml ucx -x UCX_NET_DEVICES=all --hostfile ./hostfile_$SLURM_JOBID --map-by ppr:${ppn}:node:PE=$JMP --bind-to hwthread ${binexe} config.xml | tee -a bull_log.out  
