...
df = df.groupby(["platform", "tasks"], as_index=False).agg({"energy": ['mean', 'std']})
...
for tasks in np.sort(np.unique(np.array(df['tasks']))):
    ...
    for i, line in df.loc[df['tasks'] == tasks].iterrows():
        if first:
            rects = ax.bar((platforms.index(np.array(line["platform"])[0]) * 1.5) + offset, line["energy"]["mean"], 
                            width, color=color, yerr=line["energy"]["std"], label="{}".format(tasks))
            first = False
        else:
            rects = ax.bar((platforms.index(np.array(line["platform"])[0]) * 1.5) + offset, line["energy"]["mean"], 
                            width, yerr=line["energy"]["std"], color=color)
