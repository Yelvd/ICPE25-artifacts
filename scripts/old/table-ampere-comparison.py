...
metrics = ['Runtime', 'CPI', 'Memory bandwidth', 'L3 bandwidth']
...
def load_data(results_dir, experiment=None):
    ...
    for (dirpath, dirnames, filenames) in w:
        ...
        if meta["General"]["Platform"] != platform:
            continue
...
df = df.drop(['platform', 'sum CPI'], axis=1)
...
print(df.columns)
hidelist = ["Min CPI", "Max CPI", "Min L3 BW [MB/s]", "Avg L3 BW [MB/s]", "Max L3 BW [MB/s]", "Sum L3 BW [MB/s]"]
new_df = df.drop(hidelist, axis=1)
print(new_df.style.format().hide(axis="index").to_latex(hrules=True, column_format='lrrrrrrr'))
