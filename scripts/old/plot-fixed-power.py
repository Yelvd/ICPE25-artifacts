...
df = energy.load_energy(results_dir, "fixed")

df = df.groupby(["platform", "tasks"], as_index=False).agg({"power": ['mean', 'std']}).round(2)
...
for tasks in np.sort(np.unique(np.array(df['tasks']))):
    ...
    for i, line in df.loc[df['tasks'] == tasks].iterrows():
        if first:
            rects = ax.bar((platforms.index(np.array(line["platform"])[0]) * 1.5) + offset, line["power"]["mean"], 
                            width, color=color, yerr=line["power"]["std"], label="{}".format(tasks))
            first = False
        else:
            rects = ax.bar((platforms.index(np.array(line["platform"])[0]) * 1.5) + offset, line["power"]["mean"], 
                            width, yerr=line["power"]["std"], color=color)
